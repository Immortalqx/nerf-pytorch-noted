...
def batchify(fn, chunk):
    ...
    def ret(inputs):
        # 以chunk大小分批进入网络(防止显存爆掉), 然后进行拼接
        return torch.cat([fn(inputs[i:i + chunk]) for i in range(0, inputs.shape[0], chunk)], 0)

    # chunk应该变成了ret函数中的常数, 而fn就是network
    return ret
...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024 * 64):
    ...
    :param inputs: 光线上的点, 如[1024, 64, 3]就是1024条光线, 一条光线上64个点
    :param viewdirs: viewing direction of a point in space
    :param fn: 神经网络模型, 粗糙网络或精细网络
    :param embed_fn: 坐标编码器
    :param embeddirs_fn: 方向编码器
    :param netchunk: 每个批次的大小
    :return: 网络的输出
    """
...
def render(H, W, K, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
    ...
    if use_viewdirs:
        ...
        viewdirs = viewdirs / torch.norm(viewdirs, dim=-1, keepdim=True)  # 计算单位向量
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    """
    # 这里应该是计算公式(5)中的 1-exp(-sigma_i * delta_i)
    ...
    # rays_d[..., None, :]相当于从[num_rays, 3]变成了[num_rays, 1, 3]
    # torch.norm()求范数, 结果是[num_rays, 1], 根据广播机制, 可以和dists[num_rays, num_samples]相乘
    # TODO 这一步的意义是什么
    #  如果rays_d是单位向量, 这里就是乘1, 如果不是单位向量, 这里会有什么影响?
    dists = dists * torch.norm(rays_d[..., None, :], dim=-1)

    # TODO 为什么需要用sigmoid处理一遍
    #  为了处理成0~1区间?
    ...
    # TODO 这里不是很懂, 不清楚是如何对应到论文中的公式的(右边算了个T_i)
...
def render_rays(ray_batch,
    ...
    if not lindisp:
        ...
        # TODO 不理解sample linearly in inverse depth为什么是下面这种形式
        z_vals = 1. / (1. / near * (1. - t_vals) + 1. / far * (t_vals))

    # 通过expand使N_rays的每一条光线都有的采样点
    z_vals = z_vals.expand([N_rays, N_samples])

    if perturb > 0.:
        # 前面的z_vals是均匀采样, 这里是进行分层随机采样
        # get intervals between samples
        # z_vals[..., 1:]丢弃了最左边一列(最小值), z_vals[..., :-1]同理
        # z_vals[..., :1]就是最右边的一列(最大值), z_vals[..., -1:]同理
        mids = .5 * (z_vals[..., 1:] + z_vals[..., :-1])  # 这里是把z_vals中相邻两个元素的中间值计算出来
        upper = torch.cat([mids, z_vals[..., -1:]], -1)  # mids + max
        lower = torch.cat([z_vals[..., :1], mids], -1)  # min + mids
        ...
        z_vals = lower + (upper - lower) * t_rand  # mids[i] + (mids[i+1]-mids[i])*rand
    ...
    # 在 create_nerf() 函数里面定义:(但是我不太理解为什么要这么做, 弄得这么复杂做什么?)
    # network_query_fn = lambda inputs, viewdirs, network_fn: run_network(inputs, viewdirs, network_fn,
    #                                                                     embed_fn=embed_fn,
    #                                                                     embeddirs_fn=embeddirs_fn,
    #                                                                     netchunk=args.netchunk)
...
def config_parser():
    """
    生成指定形式的参数解析器, 具体可以参考https://github.com/bw2/ConfigArgParse
    :return: 参数解析器
    """
...
def train():
    # 解析文件中的参数
    # 比如python run_nerf.py --config configs/lego.txt, 那就是解析这个lego.txt里的参数, 没有的就用默认的
    # 非常类似以前看Pi-SLAM、Map2DFusion的时候, 那个Svar
